...
<template>
  <div class="flex h-screen bg-bg text-white overflow-hidden font-sans">
    ...
    <div class="flex flex-col flex-1 overflow-hidden min-w-0">
      ...
      <main class="flex-1 overflow-y-auto p-4 md:p-6 pb-24 md:pb-6 scroll-smooth">
        <slot></slot>
      </main>
    </div>
    ...
    >
      <div 
        class="w-full bg-surface border-t border-white/10 rounded-t-3xl p-5 flex flex-col gap-3 animate-in slide-in-from-bottom duration-200"
        @click.stop
      >
        <div class="w-12 h-1 bg-white/20 rounded-full mx-auto mb-1"></div>
        <div class="flex items-center justify-between pb-2 border-b border-white/10">
          <h3 class="text-sm font-bold text-white">Ações Rápidas</h3>
